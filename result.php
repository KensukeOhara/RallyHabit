<?php
  ...
  //試合一覧の表示
  function showGameList ($dbh, $id, $affiliation) {
    $sql = 'SELECT DISTINCT `game_tb`.`game_title`, `game_tb`.`registor`,`game_tb`.`player_id1`, `player_tb`.`name`, `game_tb`.`token`, `user_tb`.`affiliation` FROM `game_tb` Inner JOIN `player_tb` on `player_id2`=`player_tb`.`id` Inner JOIN `user_tb` on `user_tb`.`name` = `game_tb`.`registor` WHERE `player_id1`="' . $id . '"';
    $sth = $dbh->query($sql);
    $results = $sth->fetchAll();
    foreach ($results as $result) {
      if($result['affiliation'] == $affiliation){
        echo '<tr>';
        echo '  <td>' . $result['game_title'] . '</td>';
        echo '  <td>' . $result['name'] . '</td>';
        echo '  <td>' . $result['registor'] . '</td>';
        if($_SESSION['user_name'] == $result['registor']){
          echo '  <td><a class = "btn btn-outline-primary" role = "button">編集</td>';
        }
        echo '</tr>';
      }
    }
...
<header>
  <nav class="navbar navbar-expand-lg navbar-light bg-light">
    <div class="container">
      <a class="navbar-brand" href="./menu.php">ラリー癖分析 (
      <?php
...
<div class="container">
  <div class="row">
    ...
    <hr>
    <div class="col-12">
      <label>試合一覧 (分析チーム: <?php echo $_SESSION['affiliation']; ?>)</label>
      <table class="table" id="game_list">
        <thead>
          ...
          <tbody>
        <?php
          showGameList($dbh, $id, $affiliation);
        ?>
          </tbody>
      </table>
    </div><!-- col-12 -->
